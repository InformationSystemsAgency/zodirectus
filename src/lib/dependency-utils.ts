...
export class DependencyUtils {
   ...
   */
  static buildDependencyGraph(results: GeneratedSchema[]): Map<string, Set<string>> {
   ...
   */
  static detectCircularDependencies(graph: Map<string, Set<string>>): string[][] {
   ...
   */
  static isCircularDependency(collectionA: string, collectionB: string, circularDeps: string[][]): boolean {
    return circularDeps.some(cycle => 
      cycle.includes(collectionA) && cycle.includes(collectionB)
    );
  }
}
