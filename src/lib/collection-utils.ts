...
export class CollectionUtils {
  ...
  static filterCollections(collections: any[], config: any): any[] {
    ...
    if (!config.includeSystemCollections) {
      targetCollections = targetCollections.filter(c => !c.collection.startsWith('directus_'));
    }

    // Filter out known folder names
    const foldersToFilter = ['Dialogues', 'Expert_System', 'Models', 'settings'];
   ...
   */
  static getFieldsToOmitForCreate(fields: any[], hasIdField: boolean): string[] {
